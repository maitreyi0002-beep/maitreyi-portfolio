...
## 2026-09-20: copy refresh, water/drop sound correction, theme-toggle anchor attempt (user)
Signature reads "Product Designer / Builder" (capitalized). The intro's second sentence is reframed: "My real interest is in how people behave and what sits underneath their intuition. The details are where that lives. The parts no one notices are the parts everyone feels." Footer water plays `drop.mp3` once per hover/focus entry, not looping — the user found a continuous loop while interacting with the water became repetitive and asked it stopped. The theme-toggle circular-reveal anchor was reworked to use the actual click position (`event.clientX/Y`) instead of the toggle's computed center, matching the standard reference implementation for this technique, but the user's reported anchor mismatch was not confirmed fixed — treat this as still potentially wrong if it comes up again, and ask for a specific description (which corner/direction the circle appears to start from) rather than re-guessing blindly.
...
## 2026-09-20: WarmCall CTA scope and Design Observability restructure (user)
The WarmCall case study's "web version" mention is now a small inline link, not a big CTA button — the page's thesis is mobile design range, so only the two mobile-related prototypes (Loop app, WarmCall mobile) get the prominent button treatment. Design Observability was converted from a hand-authored standalone page into the same generated architecture as Super Agent/Connect/WarmCall (`scripts/sync-design-observability.mjs`, driven by its existing `case-study.md`), so it now has the identical shared header, theme toggle, desktop contents sidebar, and footer. Its bespoke content (a numbered pipeline list, an evidence-type definition grid, a pull-quote) is preserved verbatim, rendered through new bespoke block handlers in that sync script rather than hand-written HTML.
