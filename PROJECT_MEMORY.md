...
## 2026-09-20: WarmCall web-prototype link demoted to inline
User clarified the WarmCall case study's whole point is showcasing mobile design range (paired deliberately with the Loop restaurant-ops app, which stays in per that same reasoning). Since only mobile matters as a "destination," the web-prototype link was pulled out of its own standalone CTA block and folded into the surrounding sentence as a normal inline link (`[text](url)` support added to `inline()` in `scripts/sync-warmcall.mjs`), leaving only the Loop and WarmCall mobile links as prominent buttons.

## 2026-09-20: Design Observability converted to the shared case-study architecture
Previously a standalone hand-written page with its own inline `<style>` block, a two-link nav, and a custom footer — no theme toggle, no contents sidebar, no shared portfolio footer. Rebuilt via `scripts/sync-design-observability.mjs`, mirroring `sync-connect.mjs`'s structure but with new block handlers specific to this page's content shapes: a `NN / label : text` pipeline pattern (rendered as `.do-pipeline`), a `Word : definition` evidence-type pattern (rendered as `.do-evidence`), and a `> ` pull-quote line (rendered as the same `<blockquote>` used elsewhere) — all driven by the page's pre-existing `case-study.md`, which needed only two small edits: the pull-quote line got a `> ` prefix, and the old page-specific "Back to the portfolio" closing line was removed since the shared footer's return links now cover that. All original wording was verified unchanged (every source sentence confirmed still present in the rendered output) — only the presentation changed. New page-specific styling lives in `site/experiments/design-observability/design-observability.css`; everything else (header, theme toggle, TOC, footer, lightbox) is the same shared CSS/JS every other case study already uses, so it needed zero page-specific code in `case-study.css`/`case-study.js`.


## 2026-09-20: custom domain (user)
User chose maitreyi.design, registered at GoDaddy, and authorized connecting the portfolio. GitHub Pages remains the host with the existing Actions workflow. Custom domain saved in repository Pages settings. GoDaddy apex A records now point to 185.199.108.153, 185.199.109.153, 185.199.110.153 and 185.199.111.153; www CNAME points to maitreyi0002-beep.github.io, all TTL 1 hour. Replaced the prior WebsiteBuilder apex target and www alias; unrelated records preserved. No CNAME file is needed with the custom Actions workflow. Public agent links and the Design Observability image source use https://maitreyi.design/. The image renderer retains legacy-URL compatibility.
