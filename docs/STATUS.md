...
The complete supplied Markdown is shipped as `site/work/super-agent/super-agent-case-study.md`, with a literal Markdown agent route at `site/work/super-agent/agents/`. The public human page excludes the agent-only Quick facts section as requested. `npm run check` and `npm run build` pass. Browser and publication verification remain to be recorded after the current implementation is inspected.
