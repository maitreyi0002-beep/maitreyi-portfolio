...
## 2026-09-20: audio scope corrections and hover sounds
Following direct user feedback, the click sound now only fires on links/buttons/the contents toggle (not every click), same-tab navigating links get a short delay so the click is audible before the page unloads, the cherry blossom hover sound was removed entirely, and hover/focus sounds were added to Selected Work rows (`bubble.mp3`) and footer contact links (`hover.mp3`). A resume link was briefly added to the footer and then removed again in the same session at the user's request; the site currently ships without one.

Validation: `npm run check` and `npm run build` passed after each change, including after two full regeneration passes of the Super Agent and Connect case-study pages (their footers are copied from the homepage and go stale when it changes). `node --check` passed on `site/app.js` throughout. No browser tool was available this session; the user tested each audio and navigation change directly in-browser and confirmed working before the next iteration, except this final push, which has not yet had a post-deploy visual/audio confirmation round.


## 2026-09-20: copy refresh, water sound fix, theme-toggle anchor attempt
Signature capitalized; intro second sentence reframed per user's supplied text. Footer water now plays `drop.mp3` once per entry (no loop, no fade-in) instead of the inaudible 1.2MB ambient loop. Theme-toggle circular reveal now anchors on the actual click position rather than the computed toggle center.

Validation: `npm run check` and `npm run build` passed; `node --check` passed on `site/app.js`. The copy change was confirmed propagating into `llms.txt` and the agent route by direct inspection. The water sound fix was confirmed working by the user in-browser across two iterations (inaudible → audible-but-repetitive → single-shot). The theme-toggle anchor fix was NOT confirmed by the user before the conversation moved on ("let it go... it is still not fixed" per the user, though phrased ambiguously) — treat this as an open item, not a resolved one, and do not claim it works without the user re-testing it.
...
Validation: build/check and diff whitespace checks passed. Browser inspected desktop light and 390px dark layouts; 320px viewport measured no horizontal overflow. Theme switch, image dialog and Escape, and literal Markdown agent navigation verified. No browser console errors observed. Shared click-sound navigation now respects event.defaultPrevented, fixing image enlargement being overridden by its delayed redirect; reproduced before and verified after on a fresh local preview. Existing reduced-motion behavior reused, no new motion added. Audio audibility and OS-level reduced-motion emulation not retested. User design approval remains pending.


## 2026-09-20: WarmCall link scope, Design Observability shared chrome
WarmCall's web-prototype mention is now an inline link, not a CTA button, leaving only the two mobile prototypes (Loop, WarmCall mobile) visually prominent. Design Observability now has the same generated header, theme toggle, desktop contents sidebar and footer as Super Agent/Connect/WarmCall, via a new `scripts/sync-design-observability.mjs`.

Validation: `npm run check` and `npm run build` passed. Every sentence from the original Design Observability page was confirmed present verbatim in the regenerated output via a script that stripped HTML and checked each source line — no wording was altered or dropped in the restructuring. HTML tag balance was verified with a parser; all new routes (case study, its agent view, its new CSS file) were confirmed serving locally. No browser tool was available this session, so visual appearance (TOC sticky behavior, pipeline/evidence grid layout, dark-mode contrast) has not been visually confirmed and awaits the user's own check.

## 2026-09-20: custom domain connected
Configured maitreyi.design in GitHub Pages and the four GitHub apex A records plus www CNAME in GoDaddy. Authoritative DNS returns all expected records. The new domain responds HTTP 200; the old GitHub Pages WarmCall URL redirects to the same path on maitreyi.design. Public agent links and the Design Observability image URL were migrated; build, check and diff whitespace checks passed. HTTPS certificate provisioning is in progress in GitHub (Certificate Requested). Enforce HTTPS must be enabled once issuance completes; do not treat secure launch as verified until then. Unrelated untracked audio files were left untouched.
