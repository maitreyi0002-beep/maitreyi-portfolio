...
const projectLinks = [...capture(/<div class="project-list">([\s\S]*?)<\/section>/).matchAll(/href="([^"]+)"/g)];
...
const summary = `# Maitreyi | Product designer\n\n## ${heading}\n${[...intro.matchAll(/<p>([\s\S]*?)<\/p>/g)].map(m=>text(m[1])).join('\n\n')}\n\n## Selected work\n${projects.map((p,i)=>`\n### ${text(p[1]).replace('↗','').trim()}\n${text(p[2])}\n[${projectLabels[i]}](${new URL(projectLinks[i][1], 'https://maitreyi.design/').href})\n`).join('')}\n## Latest experiments\n${experiments.map(m=>`[${text(m[1])}](${new URL(m[1].match(/href="([^"]+)"/)[1], 'https://maitreyi.design/').href}): ${text(m[2])}`).join('\n')}\n\n## Contact\n${contactLinks.map(m=>`- [${text(m[2])}](${m[1]})`).join('\n')}\n`;
