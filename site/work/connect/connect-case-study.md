---
title: "Connect: Turning a Hand-Held Sales Process into a Self-Serve Product"
author: Maitreyi Sharma
role: User Researcher, System Designer, UX Design
company: Loop AI (AI-powered restaurant operations platform)
timeline: June – August 2026
focus: Complex integrations & onboarding
source_url: https://uxmaitreyii.framer.website/connect
...
[image: hero — Loop Connect "Connect any data source" page on a desktop monitor]

## 0. Quick facts (for agents)

> **Image markers:** `[image: …]` shows where a product screenshot or diagram sits on the original page. Before/after pairs are marked "before" (old version) and "after" (Connect).

| Field | Value |
|---|---|
| Product | Loop Connect — the self-serve integrations and onboarding experience for getting a restaurant's data into Loop |
| My role | User Researcher, Systems & IA Designer, UI & Content Designer |
| Timeline | June – August 2026 |
| Focus | Complex integrations & onboarding |
| Research base | 2,419 customer call recordings + 183 support email threads |
| Core move | 70+ bespoke connector flows → 6 reusable patterns on one 4-step backbone, plus a "Step 0" delegation layer |
| Outcome | A people-dependent CSM process turned into a product that runs on its own: no-login delegate links, live "who-acts" status, automated access checks |

### Key numbers at a glance

| Metric | Value | Meaning |
|---|---|---|
| Integration types | 50+ (and growing) | Each set up slightly differently, by hand |
| Customers who wanted to connect vs. made it through | 68 vs. 8–10 | The old process lost most customers |
| External wizard completions | 0% (0 of 165 external visitors) | The old setup wizard was effectively an internal-only tool |
| Operators who delegate technical setup | 96% | The buyer is almost never the person doing setup |
| "Copy email" clicks | 124 clicks by 33 users | Doubt-driven repeat actions |
| Vendor export enable time | 5–7+ business days | Spinners promised seconds |
| CSM time spent chasing status | 60% of their week | Humans acting as a routing switchboard |
| Bespoke connector flows mapped | 70+ → 6 patterns | Systems work, not screen work |
| Bypass Track | 4-link, 2-week wait → 10-second paste | For users who already hold an API credential |
...
## 2. Secondary & field research: the numbers

### 2.1 Quantitative reality check

| Metric | Key insight | Strategic takeaway |
|---|---|---|
| **0%** | Zero external wizard completions | Out of 165 external users visiting the connectors page, zero completed the existing wizard. The setup wizard was effectively an internal-only tool. |
| **96%** | The delegation reality | 96% of restaurant operators who purchase Loop never execute technical setup themselves. It almost always gets passed to an IT delegate. |
| **124x** | Doubt-driven repeat actions | 33 users clicked "Copy email" 124 times, and a dedicated "I added the email again" button existed purely to soak up user uncertainty. |

### 2.2 The invisible triangle of dependencies
...
### 2.3 Four problems the research surfaced
...
## 3. Primary research: voices of users & personas

**2,419 call recordings analyzed · 183 support email threads audited**

### 3.1 The Operator / GM (96% delegation rate)
...
### 3.3 The Third-Party Vendor Rep
...
## 4. The breakthrough: the "Step 0" solution

### 4.1 Eliminating account bloat for one-time tasks
...
[image: Step 0 dialog — "Are you the admin for this connector?" with "Yes, I'm the admin" / "No, someone else manages it" and delegate name + email fields]
...
[image: diagram — the old way (forced registration) vs. the new way (Step 0 delegation engine)]

### 4.2 Three concepts for what to send the IT delegate
...
[image: standalone delegate guide — "Set up the QSR Soft connector", 3 step-locked steps with copyable Loop mailbox email, "Mark step done" and "Stuck? Talk to us"]
...
## 6. The 6 core connector patterns
...
|---|---|---|
| 1 | **Zero-Touch** (identity confirmation) | Loop already holds platform keys (e.g. Yelp, Ovation). The user or delegate just verifies account ownership and location mappings. |
| 2 | **Two-Track** (user + rep in parallel) | Quick user account access combined with a quiet, parallel vendor export ticket (e.g. Toast SFTP). |
| 3 | **Authorize-Only** (direct rep handoff) | The delegate triggers one prefilled email template; credentials flow directly from vendor rep to Loop — the customer never handles sensitive keys. |
| 4 | **Single-Invite** (add system user) | Granting dedicated user permissions to Loop (e.g. Google Business Profile). |
| 5 | **Credential Paste** (direct API keys) | Direct entry of Client IDs, Client Secrets, and Base URLs (e.g. QuickBooks, Toast API). |
| 6 | **Bypass Track** (smart branching) | Asks upfront: "Do you already hold a direct API credential?" Collapses a 4-link, 2-week vendor approval wait into an instant 10-second paste. |

### 6.1 The universal skeleton

```
Step 0: Delegate → Step 1: Grant access → Step 2: Access check → Step 3: Data ingestion → Step 4: Live
```

### 6.2 Older design versions

[image: older version — "Connect Qu" modal with an email template for the Qu account rep]

### 6.3 Current flow vs. ideal flow
...
[image: new flow 1 — Connect QSR Soft, Grant Access (delegated, "Waiting on James")]
[image: new flow 2 — Connect QSR Soft, Access Check running ("This approval can take 2 weeks")]
[image: new flow 3 — Connect QSR Soft, Data ingestion ("This step takes approx 24 hrs")]
[image: new flow 4 — Connect QSR Soft, Live ("Your connection is live")]
[image: new flow 5 — Connect QSR Soft, Share additional context]

---

## 7. Designing for human friction: small solutions

### 7.1 Explicit "who-acts" state boundaries
...
[image: who-acts state — before]
[image: who-acts state — after]

### 7.2 Time honesty & thresholds over fake spinners
...
[image: time honesty — before]
[image: time honesty — after]

### 7.3 Backend access check over self-attestation
...
[image: backend access check — before]
[image: backend access check — after]

### 7.4 Final Connect page redesign

[image: Connect page — before]
[image: Connect page — after]
